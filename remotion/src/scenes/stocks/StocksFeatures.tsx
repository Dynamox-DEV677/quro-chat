...
const FEATURES = [
  { icon: '📡', title: 'Live NSE Data', desc: '3-second real-time polling' },
  { icon: '🕯️', title: 'Candlestick Charts', desc: 'OHLC + volume bars' },
  { icon: '📈', title: 'Sparklines', desc: 'Trend at a glance' },
  { icon: '🔍', title: 'Search & Filter', desc: 'By name, symbol, or sector' },
  { icon: '📊', title: 'Market Indices', desc: 'NIFTY 50, SENSEX, BANK NIFTY' },
  { icon: '💹', title: 'Trade from Chart', desc: 'One tap to paper trade' },
];

// 9s — Stocks feature rows (staggered entrance)
export const StocksFeatures: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center', flexDirection: 'column', opacity: exitOp }}>
      <div style={{
        fontSize: 64, fontWeight: 900, color: '#f0f0f2', letterSpacing: -2, marginBottom: 80,
          ...
          return (
            <div
              ...
              style={{
                ...
                opacity: rowOp, boxShadow: '0 8px 30px rgba(0,0,0,0.3)',
              }}
            >
              <div style={{
              ...
              <div>
                <div style={{ fontSize: 38, fontWeight: 700, color: '#f0f0f2', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
                  {f.title}
                </div>
                <div style={{ fontSize: 28, color: 'rgba(240,240,242,0.45)', fontWeight: 500, marginTop: 4, fontFamily: 'system-ui, -apple-system, sans-serif' }}>
                  {f.desc}
                </div>
              </div>
            </div>
